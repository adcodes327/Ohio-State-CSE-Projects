...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        if (!m.isZero()) {
            NaturalNumber r = n.divide(m); // r holds value of remainder
            n.copyFrom(m);
            m.transferFrom(r);
            reduceToGCD(n, m);
        }
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        boolean even = false;
        n2.copyFrom(n);
        NaturalNumber r = n2.divide(two); // r hold remainder
        if (r.isZero()) { // no remainder indicates number is even
            even = true;
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        NaturalNumber two = new NaturalNumber2(2);
        NaturalNumber n2 = new NaturalNumber2(); // a copy of n
        NaturalNumber n3 = new NaturalNumber2(); // a second copy of n
        NaturalNumber r = new NaturalNumber2(); // holds the remainder for division

        n2.copyFrom(n);
        r = n2.divide(m);
        if (p.isZero() && n.isZero()) { // cases where p and n are 0
            n2.increment();
            r = n2.divide(m);
            n.copyFrom(r); // copies remainder of division
        } else {
            boolean even = isEven(p);
            n.copyFrom(r);
            if (even) { // executes when exponent is even
                p.divide(two);
                powerMod(n, p, m);
                n.power(2);
                p.multiply(two);
            } else if (!even && !p.equals(one)) { // exponent is odd and not 1
                n3.copyFrom(n);
                p.divide(two);
                powerMod(n, p, m);
                n.power(2);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        w2.copyFrom(w);
        powerMod(w2, two, n);
        if (w2.equals(one)) { // if witness is 1
            witness = true;
        }
        n2.copyFrom(n);
        n2.decrement();
        powerMod(w2, n2, n);
        if (!w2.equals(one)) { // if witness is not 1
            witness = true;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        NaturalNumber rnd = randomNumber(n); // holds result of method randomNumber

        boolean prime = true;
        int canidates = 45; // 45 is chosen # of candidates
        if (prime) {
            for (int i = 0; i < canidates; i++) {
                n2.copyFrom(n);
                n2.decrement();
                copy.increment();
                // rnd is greater than n2 & rnd is less than copy
                while (rnd.compareTo(n2) > 0 && rnd.compareTo(copy) < 0) {
                    rnd = randomNumber(n);
                }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        while (!isPrime2(n)) {
            n.increment();
            if (isEven(n)) {
                n.increment(); // increment again to get an odd number
            }
        }
    }
